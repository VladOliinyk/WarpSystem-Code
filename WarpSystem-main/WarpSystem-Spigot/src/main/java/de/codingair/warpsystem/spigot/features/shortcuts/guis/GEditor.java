...
public class GEditor extends Editor<Shortcut> {
    public GEditor(Player p, Shortcut shortcut) {
        this(p, shortcut, shortcut.clone().createDestinationIfAbsent().createTeleportSoundIfAbsent());
    }

    private GEditor(Player p, Shortcut shortcut, Shortcut clone) {
        super(p, clone, new Backup<Shortcut>(shortcut) {
                ...
                new POptions(p, clone),
                new DestinationPage(p, getMainTitle(), clone.getDestination(), Origin.ShortCut),
