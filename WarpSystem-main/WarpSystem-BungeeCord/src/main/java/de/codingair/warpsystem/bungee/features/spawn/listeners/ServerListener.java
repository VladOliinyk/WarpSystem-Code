...
public class ServerListener extends PacketListener implements Listener {
    ...
    public void onReceive(Packet packet, String extra) {
        if(packet.getType() == PacketType.SendGlobalSpawnOptionsPacket) {
            ...
            SpawnManager.getInstance().update(WarpSystem.proxy().getServerInfo(extra), p.getSpawn(), p.getRespawn());
            ...
            ServerInfo server = WarpSystem.proxy().getServerInfo(p.isRespawn() ? SpawnManager.getInstance().getRespawn() : SpawnManager.getInstance().getSpawn());
