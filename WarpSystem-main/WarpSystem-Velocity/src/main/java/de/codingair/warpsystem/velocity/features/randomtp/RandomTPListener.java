...
public class RandomTPListener extends PacketListener {
    private void add(TabCompleteEvent e, String last, String argument) {
    ...
    public void onResponse(TabCompleteEvent e) {
        ...
        if(e.getSuggestions().remove(RandomTPCompleteKeys.ID)) {
            boolean other = e.getSuggestions().remove(RandomTPCompleteKeys.ID_OTHER);
            ...
            RegisteredServer connected = con.get().getServer();
            ...
            if(start == -1 || end == -1) {
                ...
                if(semi == -1) {
                    ...
                    String server = connected.getServerInfo().getName();
                ...
                for(Player player : connected.getPlayersConnected()) {
                    if(rest.isEmpty() || player.getUsername().toLowerCase().startsWith(rest)) e.getSuggestions().add(player.getUsername());
                }
            }
        }
    }
